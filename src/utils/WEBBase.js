...
LEAPWeb.request = function (arg) {
  var e = leapclient.request2(arg);
  return e;
}


LEAPWeb.loadWebPart = function (o, b, m, c) {
  if (!o) {
    return;
  }
  if (!b) {
...
LEAPWeb.setContent = function (k, d, j, c, g) {
  var f = '<div class="part-item-content" ctf="partmodulebox"><div class="prat-item-content-box" ctf="partcontent"><img class="prat-item-loading" src="' + leapwebconfig.server + 'LEAP/Resource/HTML/indexassembly/images/loading.gif"></div><div class="prat-item-content-move" ctf="partmove"><div class="prat-item-del" ctf="partdel"></div></div></div>';
  var m = '<iframe src="@src" style="width:100%;height:100%;" frameborder="0" scrolling="no"></iframe>';
  if (!c) {
    c = window;
  }
  k.innerHTML = f;
  if (d != null) {
    var i = leapwebclient.getElement("[ctf=partcontent]", k);
    var b = leapwebclient.getElement("[ctf=partmodulebox]", k);
    var l = null;
    var e = k.getAttribute("itemmargins");
    if (e != null && e != "") {
      l = JSON.parse(e);
    }
    if (l != null) {
      ...
      if (l.bottom) {
        b.style.bottom = l.bottom;
      }
      if (l.right) {
        b.style.right = l.right;
      }
      if (l.left) {
        b.style.left = l.left;
      }
    ...
    if (d.websitetype == "1") {
      ...
      if (d.websitetype == "2") {
        i.innerHTML = m.replace("@src", d.path);
      }
    }
  }
};
